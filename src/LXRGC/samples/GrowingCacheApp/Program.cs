// LXRGC benchmark app: a realistic "growing in-memory cache" service that
...
Console.WriteLine($"# LXRGC-bench GrowingCacheApp starting: duration={durationSeconds}s label={label} finalCacheTargetMB={finalCacheTargetMB} requestAllocBytes={requestAllocBytes}");
...
while (sw.Elapsed < deadline)
{
    ...
    // actually hold in memory for the full run under LXRGC (which never
