param([int]$MaxTries = 6, [int]$Duration = 45)
...
$env:LXR_NURSERY = "1"; $env:LXR_NURSERY_DIAG = "1"
...
for ($i = 1; $i -le $MaxTries; $i++) {
    ...
    Set-Content -Path $err -Value $errtxt
    $diagLines = ($errtxt -split "`n") | Where-Object { $_ -match 'nursery-diag' }
    $missedNonZero = $diagLines | Where-Object { $_ -match 'missed=(\d+)' -and [int]$Matches[1] -gt 0 }
    Write-Host "  try $i code=$code diagLines=$($diagLines.Count) missedNonZeroReports=$($missedNonZero.Count)"
    if ($missedNonZero.Count -gt 0) {
        Write-Host "=== MISMATCH FOUND at try $i ==="
        $summary = ($errtxt -split "`n") | Where-Object { $_ -match 'nursery-diag' } | Select-Object -First 60
