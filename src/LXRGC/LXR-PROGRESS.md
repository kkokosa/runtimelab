...
| **D** | **Implicitly-dead young**: young objects with no increment are reclaimed **at the RC pause**, before decrements; young survivors (0→1 increment) **copied at that pause** to defragment (§2.1, §3.3.1–3) | **Implemented, gated OFF pending a complete barrier** (`CollectNursery`, env `LXR_NURSERY`): at each RC pause it builds a bounded young-liveness closure (roots + handles + the complete mature→young remset (F) + young→young) and reclaims young regions with no reachable young object (region-granular, Immix-style). **Root-caused a soundness blocker**: authoritative young reclaim needs a **complete** mature→young remset, which needs a write barrier capturing 100% of ref stores — our pluggable Callback barrier has residual completeness gaps (some store forms bypass it; RC/SATB tolerate this via the from-roots trace backstop, the nursery cannot). Proven by A/B: `LXR_NURSERY_FULLSCAN` (O(heap) complete seed) is **av=0** while the remset-only mode faults in the trace on a decommitted young region. Survivor copy-at-RC-pause deferred (survivors stay young, compacted by the trace-cycle `Evacuate`). **Needs a runtime change** (complete-barrier or card-marking Callback barrier → dirty-card remset scan) to enable by default. | ⚠️ |
