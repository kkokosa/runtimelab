...
| **D** | **Implicitly-dead young**: young objects with no increment are reclaimed **at the RC pause**, before decrements; young survivors (0→1 increment) **copied at that pause** to defragment (§2.1, §3.3.1–3) | **Implemented, gated OFF pending a GC-side nursery-vs-trace fix** (`CollectNursery`, env `LXR_NURSERY`): at each RC pause it builds a bounded young-liveness closure (roots + handles + the complete mature→young remset (F) + young→young) and reclaims young regions with no reachable young object. **Root cause CORRECTED (2026-07-24):** the earlier "incomplete write barrier" diagnosis was **wrong**. A crash-safe classification diagnostic (`LXR_NURSERY_DIAG` → `RunNurseryDiag`) with a **from-roots full-reachability probe** proves every young object the remset closure "misses" (vs a full-heap scan) is **unreachable from roots = dead** (`missedReachableFromRoots=0/17`, `0/138`), so the **remembered set / barrier is complete**; `LXR_NURSERY_FULLSCAN` only *masked* the AV by conservatively over-retaining dead young (fewer decommits). Also `youngMarked=0` at RC pauses. **No runtime change is needed.** The residual nursery AV is a **GC-side nursery-vs-trace reclamation/recycling race** (a decommitted dead-young region later touched by the trace/RC path), to be fixed alongside **C**. Also note this remset-closure nursery is itself a **deviation from the paper**, which reclaims implicitly-dead young via **RC=0 at the RC pause** (young are currently RC-*skipped* in `RCIncrement`/`RCDecrement`); the faithful redesign is RC-counted young + implicitly-dead-at-RC=0 (reuses the modbuf + root deferral (B), no remembered set). Survivor copy-at-RC-pause deferred (survivors stay young, compacted by the trace-cycle `Evacuate`). D stays ⚠️. |
