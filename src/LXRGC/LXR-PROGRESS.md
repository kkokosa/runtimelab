...
- **2026-07-24c** — **Root-caused & fixed the frequent-trigger storm AV (~50% of
  ...
  `-tagb 3` storms the full concurrent config AV'd ~50% of runs in the concurrent
  marker (`LXRBigArrayScanFn` / `DrainSliceLocal`). Diagnosis (this session): a new
  `LXR_NURSERY_GUARD` captured the exact parent→child edge into every freed-young
  region instead of faulting, and a `FreedYoungRange.site`/epoch tag disambiguated
  the reclaimer. Ground truth: a **marked** (grey/live) parent (big reference array,
  `parentMARKED=1`) references a young child whose region a nursery reclaimer freed;
  the child's `childRC` was ≥1 (NOT an RC-undercount) and `[verify-nursery-copy]`
  reported **`UNFORWARDED ref referrer=<big array> marked=1 rc=1 bornEp≪gEp
  inRemset=0`** immediately before every AV. Root cause: `CopyYoungSurvivors`
  (RC-pause D-copy, a **non-paper** extension) forwards referrers via the SCOPED
  old→young remembered set (`g_dcopyModifiedSlots`), which provably cannot capture a
  **stable** mature→young edge (a big-array element stored in an earlier epoch, its
  coalescing first-log long cleared → `inRemset=0`, never re-logged). Freeing/moving
  a survivor under such an unforwarded straggler dangles the referrer → the marker
  dereferences freed/reused memory → AV. Fix (paper-faithful, no shortcut): the paper
  does copying/defrag ONLY in the STW **trace** (`Evacuate`, item F) where mark
  authority forwards EVERY marked referrer completely; RC pauses only reclaim
  implicitly-dead young (`CollectNursery`). So `CopyYoungSurvivors` is now
  **default-OFF** (`LXR_NURSERY_COPY=1` opt-in), and its eager source-free is
  additionally gated behind `LXR_NURSERY_COPY_EAGER_FREE=1` (deferring to the
  mark-authoritative reclaimers is the sound reconciliation). **Verified: 0 AV / 12
  storm runs** (default), ConsoleApp default-config smoke exit 0 + `LXR_VERIFY_TRACE`
  clean; experiments confirmed `LXR_NURSERY_COPY=0` → 0/8, `LXR_NURSERY_COPY_NO_FREE`
  → 0/6, default D-copy-on → 3/6 AV. Sound storm footprint ≈ 1.4 GB (follow-up:
  extend `Evacuate` to select fragmented **young** regions as evacuation candidates
  so young defrag rides the mark-authoritative trace, recovering footprint toward the
  crashing baseline's ~1.06 GB / Server's ~0.5 GB). No runtime change.
- **2026-07-28s** — **Reverted the "Option B" concurrent young reclamation as a
  paper-UNFAITHFUL deviation; re-grounded the reclamation model in the paper (§1):
  ALL memory freeing happens at STW RC pauses.** Chasing the footprint runaway I had
  added an experiment (`CollectNursery` freeing dead-young lines/blocks **off-pause,
  during an open trace window**). It intermittently AV'd (~1 crash / storm iter):
  captured native stack = a **parallel mark worker** (`LXRBigArrayScanFn →
  LXRMarkWorkerProc`) faulting on a **READ of a decommitted region**
  (`chunk[946] committed=0`, `phase=idle`) — the freed young region was recycled/
  decommitted and read via a stale mark-stack pointer (address recycling). Re-read
  the paper (ar5iv §1 Design) and confirmed the experiment was the reinvention, not
  the bug: *"It copies only during stop-the-world pauses"*; *"At each pause, LXR
  performs reference counting increments and reclaims free blocks and lines with dead
  young objects…"*; *"After the trace completes, LXR reclaims free blocks and lines at
  the next RC pause."* → **all reclamation is pause-only; only SATB tracing + lazy
  decrements are concurrent, and LXR's trace workers ARE the pause workers (naturally
  quiesced whenever memory is freed).** Our architectural mismatch is that our
  concurrent marker is a **separate thread not suspended by `SuspendEE`** (which stops
  only mutators) AND our trace is not pure SATB (it re-scans roots + new-values to
  cover .NET roots that move without a barrier), so it CAN dereference post-snapshot
  young — which is exactly what off-pause young reclaim raced. **Fix:** `CollectNursery`
  now **defers** young reclaim whenever `g_traceWindowOpen` (the paper's "consume trace
  results at the next pause" rule); the off-pause experiment is retained only behind an
  opt-in `LXR_NURSERY_DURING_TRACE=1`. All other RC reclaimers already correctly deferred
  during the window (`ReclaimMatureByRC`; sweep/evac at finish) — only the experiment
  broke the rule. Also added a paper-faithful **trace-window-length cap** lever
  (`LXR_TRACE_MAX_SPAN`, default 3) that force-finishes an over-long multi-epoch window
  so safe finish-pause reclamation runs frequently ("regular, brief stop-the-world
  collections"); NOTE it engages only on the `g_multiEpoch` path, NOT the production
  `useConcurrent` path (still inert there). **Verified:** storm loop **6/6 exit 0**
  (was crashing iter 1) + ConsoleApp reclamation smoke clean (exit 0, peak 121 MB,
  committed 53 MB). **Footprint status (still open):** baseline stable config peaks
  ~2.7 GB — NOT yet bounded; `LXR_GC_GROWTH_PCT=0` reaches ~1 GB but hits a
  **pre-existing** RC line-carve/STW correctness hazard (`0x80131506` in `DoSurvive`
  after a large `CarveDeadRunsByRC` burst), unrelated to the reverted experiment. The
  paper-faithful footprint win requires fixing that pre-existing carve hazard and/or
  pacing the `useConcurrent` cadence (survival-rate predictor); the primary fidelity
  fix (reclaim only at pauses) is delivered.
